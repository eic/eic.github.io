...
- **Opening a full dataset with Python:**
  
  Using uproot to create a histogram of `EventHeader.eventNumber`:
  ```python
  from rucio.client import Client
  import uproot
  import numpy as np
  ...
  event_numbers = np.concatenate([
      uproot.open(f"{file_path}:events")["EventHeader.eventNumber"].array()
      for file_path in file_paths
  ])
  
  # Create and save a histogram
  plt.hist(event_numbers, bins=100)
  plt.xlabel("Event Number")
  plt.title("EventHeader.eventNumber")
  plt.savefig("event_numbers.png")
  ```

  Using ROOT with TChain to create a histogram of `EventHeader.eventNumber`:
  ```python
  from rucio.client import Client
  import ROOT
  ...
  # Create a TChain with all files
  chain = ROOT.TChain("events")
  for file_path in file_paths:
      chain.Add(file_path)
  
  # Draw a histogram of EventHeader.eventNumber (auto-range)
  canvas = ROOT.TCanvas("c", "EventHeader.eventNumber")
  chain.Draw("EventHeader.eventNumber")
  canvas.SaveAs("event_numbers.png")
  ```

  Using ROOT RDataFrame to create a histogram of `EventHeader.eventNumber`:
  ```python
  from rucio.client import Client
  import ROOT
  ...
  # Create an RDataFrame with all files and draw a histogram of EventHeader.eventNumber
  rdf = ROOT.RDataFrame("events", file_paths)
  h = rdf.Histo1D("EventHeader.eventNumber")
  canvas = ROOT.TCanvas("c", "EventHeader.eventNumber")
  h.Draw()
  canvas.SaveAs("event_numbers.png")
  ```
